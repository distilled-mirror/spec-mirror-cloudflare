...
[Transfer In Status](https://developers.cloudflare.com/api/resources/registrar/subresources/transfer_in_status)
...
Use this endpoint to poll transfer progress after initiating a transfer with `POST /accounts/{account_id}/registrar/registrations/{domain_name}/transfer-in`. The URL is provided in the `links.self` field of the transfer response.
...
- `in_progress`: Transfer has been submitted to the registry and is being processed. Continue polling.
...
- `action_required`: The user needs to take action (e.g., the FOA email needs to be accepted). See `context` for details on what is needed.
...
- `blocked`: The transfer is waiting on a third party (e.g., losing registrar approval). Poll every 30–60 minutes.
- `action_required`: Stop polling. The workflow will not advance until the user takes action. Check `context` for details on what is needed.
...
[Link to this property](#)%20registrar.transfer_in_status%20%3E%20(method)%20get%20%3E%20(params)%20default%20%3E%20(param)%20account_id%20%3E%20(schema)>)
...
[Link to this property](#)%20registrar.transfer_in_status%20%3E%20(method)%20get%20%3E%20(params)%20default%20%3E%20(param)%20domain_name%20%3E%20(schema)>)
...
[Link to this property](#)%20registrar.transfer_in_status%20%3E%20(method)%20get%20%3E%20(network%20schema)%20%3E%20(property)%20errors>)
...
[Link to this property](#)%20registrar.transfer_in_status%20%3E%20(method)%20get%20%3E%20(network%20schema)%20%3E%20(property)%20messages>)
...
[Link to this property](#)%20registrar.transfer_in_status%20%3E%20(method)%20get%20%3E%20(network%20schema)%20%3E%20(property)%20result>)

success: true

Whether the API call was successful.

[Link to this property](#)%20registrar.transfer_in_status%20%3E%20(method)%20get%20%3E%20(network%20schema)%20%3E%20(property)%20success>)
...
```
curl https://api.cloudflare.com/client/v4/accounts/$ACCOUNT_ID/registrar/registrations/$DOMAIN_NAME/transfer-in-status \
...
```
{
  ...
  "result": {
    ...
    "context": {
      "domain_name": "example.com",
      "transfer_status": "awaiting_approval"
    },
    "created_at": "2025-10-27T10:00:00Z",
    "links": {
      "resource": "/accounts/abc/registrar/registrations/example.com",
...
```
{
  ...
  "result": {
    ...
    "state": "succeeded",
    "updated_at": "2025-10-30T08:15:00Z"
  },
  "success": true
}
```
